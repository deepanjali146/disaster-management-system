...
def fetch_weather_data(location):
    ...
    try:
        ...
        area = nearest0.get('areaName')
        if area and len(area) > 0:
            real_location = area[0].get('value', location)
        else:
...
def allocate_team():
    ...
    return redirect(url_for("government_dashboard"))
def _team_type_from_unit_category(unit_category):
    """Map emergency_units.unit_category to emergency_assignments.team_type."""
    mapping = {
        "Rescue": "Rescue",
        "Escort": "Escort",
        "Medical": "Liaison",
        "ResourceCollector": "Liaison",
        "FoodSupply": "FoodSupply",
    }
    return mapping.get(unit_category, "Rescue")


@app.route("/assign_emergency_team", methods=["POST"])
@require_role("government")
@handle_errors("government_dashboard", "Assign emergency team failed:")
def assign_emergency_team():
    """Government allocates a rescue unit to an emergency request."""
    request_id = request.form.get("request_id")
    unit_id = request.form.get("unit_id")

    if not request_id or not unit_id:
        flash("Request and emergency team are required", "danger")
        return redirect(url_for("government_dashboard"))
    ...
    try:
        unit_resp = supabase.table("emergency_units").select(
            "id, unit_name, unit_category, head_id, status"
        ).eq("id", int(unit_id)).limit(1).execute()

        if not unit_resp or not unit_resp.data:
            flash("Emergency team not found", "danger")
            return redirect(url_for("government_dashboard"))

        unit = unit_resp.data[0]
        if not unit.get("head_id"):
            flash("This team has no team lead assigned. Add a head to the unit first.", "danger")
            return redirect(url_for("government_dashboard"))

        if (unit.get("status") or "").lower() == "busy":
            flash(f"{unit.get('unit_name')} is already on another assignment.", "warning")
            return redirect(url_for("government_dashboard"))

        req_resp = supabase.table("requests").select(
            "id, status, incident_id, incidents(location, pincode)"
        ).eq("id", int(request_id)).limit(1).execute()
        ...
        incident = request_row.get("incidents") or {}
        location_text = incident.get("location")

        assignment_payload = {
            "request_id": int(request_id),
            "unit_id": int(unit_id),
            "team_lead_id": unit["head_id"],
            "team_name": unit["unit_name"],
            "team_type": _team_type_from_unit_category(unit.get("unit_category")),
            "location_text": location_text,
            "status": "Assigned",
            "assigned_at": datetime.now().isoformat(),
        }

        ins = supabase.table("emergency_assignments").insert(assignment_payload).execute()
        if not ins or not ins.data:
            flash("Could not create team assignment.", "danger")
            return redirect(url_for("government_dashboard"))

        supabase.table("emergency_units").update({
            "status": "Busy",
        ...
        try:
            supabase.table("emergency_notifications").insert({
                "request_id": int(request_id),
                "gov_id": session.get("user_id"),
        ...
        flash(
            f"{unit['unit_name']} assigned to request #{request_id} at {location_text or 'the incident location'}.",
            "success",
        )

    except Exception as e:
        flash(f"Error assigning team: {e}", "danger")
    ...
    try:
        ...
        for unit_id in unit_ids:
            ...
            new_assignment_payload = {
                ...
                "team_lead_id": unit_data.get('head_id') or assigned_head['id'],
                "unit_id": int(unit_id),
                "team_name": unit_data.get('unit_name') or 'Emergency Unit',
                "team_type": _team_type_from_unit_category(unit_data.get('unit_category')),
                "location_text": original_incident.get('location'),
                "status": "Assigned",
                "assigned_at": datetime.now().isoformat(),
